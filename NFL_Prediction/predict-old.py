...
names_tst_large = np.ndarray(names_tst.shape, basestring)
names_tst_small = np.ndarray(names_tst.shape, basestring)
X_tst_poly_large = np.zeros(X_tst_poly.shape)
X_tst_poly_small = np.zeros(X_tst_poly.shape)
...
names_trn_large = np.ndarray(names_trn.shape, basestring)
names_trn_small = np.ndarray(names_trn.shape, basestring)
X_trn_poly_large = np.zeros(X_trn_poly.shape)
X_trn_poly_small = np.zeros(X_trn_poly.shape)
...
while tst_count < len(spread_tst):
    if abs(spread_tst[tst_count]) >= 5:
        ...
        X_tst_poly_large[large_tst_count,:] = X_tst_poly[tst_count,:]
        ...
        X_tst_poly_small[small_tst_count,:] = X_tst_poly[tst_count,:]
...
while trn_count < len(spread_trn):
    if abs(spread_trn[trn_count]) >= 5:
        ...
        X_trn_poly_large[large_trn_count,:] = X_trn_poly[trn_count,:]
        ...
        X_trn_poly_small[small_trn_count,:] = X_trn_poly[trn_count,:]
...
pandas.DataFrame(np.dot(X_trn_poly_small.T, X_trn_poly_small)).to_csv("junk.csv")
pandas.DataFrame(np.dot(X_trn_poly_large.T, X_trn_poly_large)).to_csv("junk-2.csv")
...
pandas.DataFrame([names_tst_large, spread_tst_large, y_tst_large, y_pred_tst_linear_large, cover_tst_large, \
                 cover_pred_tst_linear_large, names_tst_small, spread_tst_small, y_tst_small, y_pred_tst_linear_small, \
...
# win_trn = np.sign(y_trn)
# win_tst = np.sign(y_tst)
# win_spread_trn_pred = -1* np.sign(spread_trn)
# win_spread_tst_pred = -1*np.sign(spread_tst)
# win_trn_linear_pred = np.sign(y_pred_trn_linear)
# win_trn_poly_pred = np.sign(y_pred_trn_poly)
# win_tst_linear_pred = np.sign(y_pred_tst_linear)
# win_tst_poly_pred = np.sign(y_pred_tst_poly)
#
# large_trn_linear_upset_correct = 0
# large_trn_linear_upset_incorrect = 0
# large_trn_poly_upset_correct = 0
# large_trn_poly_upset_incorrect = 0
# large_tst_linear_upset_correct = 0
# large_tst_linear_upset_incorrect = 0
# large_tst_poly_upset_correct = 0
# large_tst_poly_upset_incorrect = 0
# small_trn_linear_upset_correct = 0
# small_trn_linear_upset_incorrect = 0
# small_trn_poly_upset_correct = 0
# small_trn_poly_upset_incorrect = 0
# small_tst_linear_upset_correct = 0
# small_tst_linear_upset_incorrect = 0
# small_tst_poly_upset_correct = 0
# small_tst_poly_upset_incorrect = 0
#
# for i in range(len(win_trn_linear_pred)):
#     if win_spread_trn_pred[i] != win_trn_linear_pred[i]:
#         if win_trn[i] == 0:
#             large_trn_linear_upset_correct += 0
#         elif win_trn[i] == win_trn_linear_pred[i]:
#             if spread_trn[i] * np.sign(spread_trn[i]) >= 4:
#                 large_trn_linear_upset_correct += 1
#             else:
#                 small_trn_linear_upset_correct += 1
#         else:
#             if spread_trn[i] * np.sign(spread_trn[i]) >= 4:
#                 large_trn_linear_upset_incorrect += 1
#             else:
#                 small_trn_linear_upset_incorrect += 1
#
# for i in range(len(win_trn_poly_pred)):
#     if win_spread_trn_pred[i] != win_trn_poly_pred[i]:
#         if win_trn[i] == 0:
#             large_trn_poly_upset_correct += 0
#         elif win_trn[i] == win_trn_poly_pred[i]:
#             if spread_trn[i] * np.sign(spread_trn[i]) >= 5:
#                 large_trn_poly_upset_correct += 1
#             else:
#                 small_trn_poly_upset_correct += 1
#         else:
#             if spread_trn[i] * np.sign(spread_trn[i]) >= 5:
#                 large_trn_poly_upset_incorrect += 1
#             else:
#                 small_trn_poly_upset_incorrect += 1
#
# for i in range(len(win_tst_linear_pred)):
#     if i<88:
#         if win_spread_tst_pred[i] != win_tst_linear_pred[i]:
#             if win_tst[i] == 0:
#                 large_tst_linear_upset_correct += 0
#             elif win_tst[i] == win_tst_linear_pred[i]:
#                 if spread_tst[i] * np.sign(spread_tst[i]) >= 4:
#                     large_tst_linear_upset_correct += 1
#                 else:
#                     small_tst_linear_upset_correct += 1
#             else:
#                 if spread_tst[i] * np.sign(spread_tst[i]) >= 4:
#                     large_tst_linear_upset_incorrect += 1
#                 else:
#                     small_tst_linear_upset_incorrect += 1
#
# for i in range(len(win_tst_poly_pred)):
#     if i<88:
#         if win_spread_tst_pred[i] != win_tst_poly_pred[i]:
#             if win_tst[i] == 0:
#                 large_tst_poly_upset_correct += 0
#             elif win_tst[i] == win_tst_linear_pred[i]:
#                 if spread_tst[i] * np.sign(spread_tst[i]) >= 5:
#                     large_tst_poly_upset_correct += 1
#                 else:
#                     small_tst_poly_upset_correct += 1
#             else:
#                 if spread_tst[i] * np.sign(spread_tst[i]) >= 5:
#                     large_tst_poly_upset_incorrect += 1
#                 else:
#                     small_tst_poly_upset_incorrect += 1
#
# trn_linear_upset_correct = large_trn_linear_upset_correct + small_trn_linear_upset_correct
# trn_linear_upset_incorrect = large_trn_linear_upset_incorrect + small_trn_linear_upset_incorrect
# tst_linear_upset_correct = large_tst_linear_upset_correct + small_tst_linear_upset_correct
# tst_linear_upset_incorrect = large_tst_linear_upset_incorrect + small_tst_linear_upset_incorrect
# trn_poly_upset_correct = large_trn_poly_upset_correct + small_trn_poly_upset_correct
# trn_poly_upset_incorrect = large_trn_poly_upset_incorrect + small_trn_poly_upset_incorrect
# tst_poly_upset_correct = large_tst_poly_upset_correct + small_tst_poly_upset_correct
# tst_poly_upset_incorrect = large_tst_poly_upset_incorrect + small_tst_poly_upset_incorrect
#
# upset_trn_linear_percent = float(trn_linear_upset_correct) / (
#             float(trn_linear_upset_correct) + float(trn_linear_upset_incorrect))
#
# upset_trn_poly_percent = float(trn_poly_upset_correct) / (
#             float(trn_poly_upset_correct) + float(trn_poly_upset_incorrect))
#
# upset_tst_linear_percent = float(tst_linear_upset_correct) / (
#             float(tst_linear_upset_correct) + float(tst_linear_upset_incorrect))
#
# upset_tst_poly_percent = float(tst_poly_upset_correct) / (
#             float(tst_poly_upset_correct) + float(tst_poly_upset_incorrect))
#
# large_trn_linear_upset_percent = float(large_trn_linear_upset_correct) / (float(large_trn_linear_upset_correct) + float(large_trn_linear_upset_incorrect))
# large_trn_poly_upset_percent = float(large_trn_poly_upset_correct) / (float(large_trn_poly_upset_correct) + float(large_trn_poly_upset_incorrect))
# large_tst_linear_upset_percent = float(large_tst_linear_upset_correct) / (float(large_tst_linear_upset_correct) + float(large_tst_linear_upset_incorrect))
# large_tst_poly_upset_percent = float(large_tst_poly_upset_correct) / (float(large_tst_poly_upset_correct) + float(large_tst_poly_upset_incorrect))
#
# small_trn_linear_upset_percent = float(small_trn_linear_upset_correct) / (float(small_trn_linear_upset_correct) + float(small_trn_linear_upset_incorrect))
# small_trn_poly_upset_percent = float(small_trn_poly_upset_correct) / (float(small_trn_poly_upset_correct) + float(small_trn_poly_upset_incorrect))
# small_tst_linear_upset_percent = float(small_tst_linear_upset_correct) / (float(small_tst_linear_upset_correct) + float(small_tst_linear_upset_incorrect))
# small_tst_poly_upset_percent = float(small_tst_poly_upset_correct) / (float(small_tst_poly_upset_correct) + float(small_tst_poly_upset_incorrect))
#
# for i in range(len(spread_tst)):
#     if spread_tst[i] * np.sign(spread_tst[i])>=5:
#         spread_tst[i] = 0
#         y_pred_tst_linear[i] = 0
#         cover_tst[i] = 0
#         cover_pred_tst_linear[i] = 0
#
# pandas.DataFrame([-1*spread_tst, y_pred_tst_linear, y_tst, cover_tst, cover_pred_tst_linear]).to_csv(
#     "junk.csv")
#
# # print('The percent training upset correct using linear is: ', upset_trn_linear_percent)
# # print('The percent training upset correct using poly is: ', upset_trn_poly_percent)
# # print('The percent testing upset correct using linear is: ', upset_tst_linear_percent)
# # print('The percent testing upset correct using poly is: ', upset_tst_poly_percent)
# #
# # print('The percent training upset correct using linear with large spread is: ', large_trn_linear_upset_percent)
# # print('The percent training upset correct using poly with large spread is: ', large_trn_poly_upset_percent)
# # print('The percent testing upset correct using linear with large spread is: ', large_tst_linear_upset_percent)
# # print('The percent testing upset correct using poly with large spread is: ', large_tst_poly_upset_percent)
# #
# # print('The percent training upset correct using linear with small spread is: ', small_trn_linear_upset_percent)
# # print('The percent training upset correct using poly with small spread is: ', small_trn_poly_upset_percent)
# # print('The percent testing upset correct using linear with small spread is: ', small_tst_linear_upset_percent)
# # print('The percent testing upset correct using poly with small spread is: ', small_tst_poly_upset_percent)
# # print(large_trn_linear_upset_correct, large_trn_linear_upset_incorrect)
# # print(large_trn_poly_upset_correct, large_trn_poly_upset_incorrect)
# # print(large_tst_linear_upset_correct, large_tst_linear_upset_incorrect)
# # print(large_tst_poly_upset_correct, large_tst_poly_upset_incorrect)
# # print(small_trn_linear_upset_correct, small_trn_linear_upset_incorrect)
# # print(small_trn_poly_upset_correct, small_trn_poly_upset_incorrect)
# # print(small_tst_linear_upset_correct, small_tst_linear_upset_incorrect)
# print(small_tst_poly_upset_correct, small_tst_poly_upset_incorrect)